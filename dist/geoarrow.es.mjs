...
import { DataType, Vector, BufferType, Data, Type, Duration, Map_, FixedSizeList, FixedSizeBinary, Union, Struct, List, Interval, Timestamp, Time, Date_, Decimal, Bool, Utf8, Binary, Float, Int, Null, Field, makeData, Float64 } from 'apache-arrow';
...
function parseWkb(data, type, dim) {
    ...
    switch (type) {
        ...
        case WKBType.Polygon:
            return repackPolygons(parsedGeometries, dim);
        default:
...
function repackPoints(geoms, dim) {
    ...
    return makeData({
        type: new FixedSizeList(dim, new Field(coordFieldName(dim), new Float64(), false)),
        child: coordsData,
    });
}
function repackLineStrings(geoms, dim) {
    ...
    const verticesData = makeData({
        type: new FixedSizeList(dim, new Field(coordFieldName(dim), coordsData.type, false)),
        child: coordsData,
    });
    return makeData({
